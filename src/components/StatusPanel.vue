  ...
  <section class="card rise p-5 sm:p-6">
    <div class="flex flex-wrap items-start gap-4">
      ...
      <div class="min-w-[15rem] flex-1">
        ...
        <h2 class="mt-1 text-[1.25rem] text-ink">{{ hold.title }}</h2>
        <p class="mt-1.5 max-w-[46ch] text-[0.9375rem] leading-snug text-mute">{{ hold.detail }}</p>
      </div>
    ...
    >
      <div>
        ...
        <dd class="num mt-0.5 text-[1.125rem] text-ink">{{ elapsed(elapsedMs) }}</dd>
      </div>
      <div>
        <dt class="label text-soft">Screen lock</dt>
        <dd class="num mt-0.5 text-[1.125rem] text-ink">{{ elapsedShort(lockedMs) }}</dd>
      </div>
      <div>
        <dt class="label text-soft">In the background</dt>
        <dd class="num mt-0.5 text-[1.125rem] text-ink">{{ elapsedShort(backgroundMs) }}</dd>
      </div>
      <div>
        <dt class="label text-soft">Today</dt>
        <dd class="num mt-0.5 text-[1.125rem] text-ink">{{ elapsed(dayMs) }}</dd>
      </div>
    </dl>
    ...
    <p v-for="problem in problems" :key="problem" class="mt-3 text-[0.875rem] leading-snug text-soft">
      {{ problem }}
    </p>

    <label class="field-row mt-4">
      <span>
        <span class="ui text-[0.9375rem] font-medium text-ink">Mini window</span>
    ...
    <p class="mt-4 text-[0.875rem] leading-snug text-soft">
