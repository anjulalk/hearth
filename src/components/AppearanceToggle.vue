...
/** The footer's appearance control: Inter, 3rem square, ink on hover (DESIGN.md, Footer). */
...
<template>
  <button
    type="button"
    class="ui flex h-12 w-12 shrink-0 cursor-pointer items-center justify-center text-mute transition-colors hover:text-clay-strong"
    aria-label="appearance switcher"
    :title="`${label}. Right click follows the system.`"
    @click="store.toggleAppearance()"
    @contextmenu.prevent="store.resetAppearance()"
  >
    <svg v-if="dark" class="h-5 w-5" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
    ...
    <svg v-else class="h-5 w-5" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
