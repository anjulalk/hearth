...
/** The appearance control, beside the wordmark in the header. */
...
<template>
  <button
    type="button"
    class="ui flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-full text-mute transition-colors hover:text-clay"
    aria-label="appearance switcher"
    :title="`${label}. Right click follows the system.`"
    @click="store.toggleAppearance()"
    @contextmenu.prevent="store.resetAppearance()"
  >
    <svg v-if="dark" class="h-[1.125rem] w-[1.125rem]" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
    ...
    <svg v-else class="h-[1.125rem] w-[1.125rem]" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
